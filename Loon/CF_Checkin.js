...
const userAgent =
  "Mozilla/5.0 (iPhone; CPU iPhone OS 16_6 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 cfmoto/1.0.0";
const headers = {
  "Content-Type": "application/json",
  cookie: cookie,
  "user-agent": userAgent,
};
const url = "https://c.cfmoto.com";
$.signInfo = {};
...
function sign() {
  return new Promise((resolve) => {
    const option = {
      url: url + "/cfmotoservermall/app/integral/task/complete/v1",
      headers,
      body: {
        completeStatu: 1,
        taskDetail: 8,
...
async function getSignInfo() {
  const option = {
    url: url + "/cfmotoservermall/app/user/integral/current/user/info",
    headers,
  };
...
async function finishTask() {
  // 创建帖子
  const postId = await createComment();
  // 评论帖子
  await postComments(postId);
  // 点赞
...
function createComment() {
  return new Promise((resolve) => {
    ...
    $httpClient.post(option, (error, res, data) => {
...
function postComments(postId) {
  return new Promise((resolve) => {
    const option = {
      url: url + "/jv/bbs/article/comment-v1/",
      headers,
      body: {
        post_id: postId,
        content: "666",
...
function thumbsUp(postId) {
  return new Promise((resolve) => {
    const option = {
      url: url + "/jv/bbs/post/thumbs_up/" + postId,
      headers,
      body: {},
    };
    $httpClient.post(option, (error, res, data) => {
...
function share() {
  return new Promise((resolve) => {
    const option = {
      url: url + "/cfmotoservermall/app/integral/task/complete",
      headers,
      body: {
        completeStatu: 1,
        taskDetail: 13,
...
function deletePost(postId) {
  return new Promise((resolve) => {
    const option = {
      url: url + "/jv/bbs/post/?id=" + postId,
      headers,
      body: {},
    };
    $httpClient.delete(option, (error, res, data) => {
      return resolve();
    });
  });
}
async function showMsg() {
  if ($.signInfo.isSign) {
    $.msg(
      title,
      "✅ 今日任务已完成",
      "当前累计积分：" + $.signInfo.integralTotal + "分",
    );
  } else {
    $.msg(
      title,
      "🎉 今日任务已完成，获取积分：" + ($.signInfo.integral + 36) + "分",
      "当前累计积分：" + $.signInfo.integralTotal + "分",
    );
  }
}
